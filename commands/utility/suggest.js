module.exports = {
  name: "saran",
  description:
    "Beri saran atau laporkan bug terkait ⚓ BaquaBot pada pengembang",
  ...
  async execute(client, message, args) {
    ...
    if (message.source.type !== "group" && message.type !== "text")
      return message.reply("Hanya berfungsi di group");
    let userId = message.source.userId;
    client.getUserProfile(userId).then((profile) => {
      client.push(
        process.env.dev_id,
        `Dari ${profile.displayName}\n\n${saran}`
      );
