...
module.exports = {
    name: 'danbooru',
    description: 'Random image dan safe rating via danbooru, i dont want to get banned',
    aliases : [],
    category: 'Anime',
    ...
    if(!tags) return message.reply('Input tidak terdeteksi!')
     ...
     await axios.get(`http://danbooru.donmai.us/posts.json?limit=100&tags=${encodeURI(`${tags}+rating:s}`)}`)
     .then((res)=> {
       let random = res.data[Math.floor(Math.random()*res.data.length)]
        if (random.id === undefined || random.file_url === undefined) return message.reply('Couldn\'t fetch image, sorry try again :)')
       let imgurl = random.file_url
       axios({url: imgurl, responseType:'stream'})
       .then((img)=>{
         img.data.pipe(fs.createWriteStream(`./public/danbooru/${random.id}.jpg`))
    ...
    if (random.rating === "u") random.rating === "Unrated";
         ...
         message.reply([
           ...
           {
             type:'image',
             originalContentUrl:`https://tamaline.tama0612.repl.co/img/danbooru/${random.id}.jpg`,
             previewImageUrl:`https://tamaline.tama0612.repl.co/img/danbooru/${random.id}.jpg`
           }
         ])
       })
