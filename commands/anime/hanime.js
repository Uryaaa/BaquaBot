...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!res.hits)
      return message.reply(`Tidak ada ditemukan hentai dengan query ${query}`);
...
tags : ${res.videos[0].tags
          .sort()
          .map((x) => x)
          .join(", ")}

Released : ${moment(new Date(res.videos[0].released_at * 1000)).format(
          "dddd, do MMMM YYYY"
        )}
...
▶ Watch ${
          res.videos[0].is_censored ? "CENSORED" : "UNCENSORED"
        } on https://hanime.tv/videos/hentai/${res.videos[0].slug}`,
      },
    ]);
  },
};
