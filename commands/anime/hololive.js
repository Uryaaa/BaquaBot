...
module.exports = {
  ...
  async execute(client, message, args) {
    axios
      .get(`https://www.reddit.com/r/hololive/random.json`)
      .then((results) => {
        //console.log(results.data[0].data.children[0].data)
        let memetype = results.data[0].data.children[0].data;
        if (
          memetype.url.endsWith("jpg") ||
          memetype.url.endsWith("png") ||
          memetype.url.endsWith("gif")
        ) {
          message.reply([
            {
              type: "text",
            ...
            message.reply([
              {
                type: "video",
                originalContentUrl:
                  memetype.media.reddit_video.scrubber_media_url,
              },
              {
                type: "text",
                text: `${memetype.title} submitted by ${memetype.author}\n\nLink to post :  https://www.reddit.com${memetype.permalink}\nContent : ${memetype.url}`,
